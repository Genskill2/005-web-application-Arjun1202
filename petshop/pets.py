...
def search(field, value):
    conn=db.get_db()
    cursor=conn.cursor()
    cursor.execute(f"select p.id,p.name,p.sold,an.name from pet p,tag tg1,tag_pets tg2,animal an where tg1.name='{value}' and tg2.tag=tg1.id and tg2.pet=p.id and an.id=p.species")
    q=cursor.fetchall()
